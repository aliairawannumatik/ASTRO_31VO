    ...
    <div className={`relative min-h-screen flex flex-col items-center overflow-hidden ${isLight ? "gradient-snow" : "gradient-space"}`}>
      ...
      <div className="relative z-10 w-full max-w-5xl px-4 py-6 flex flex-col items-center">
        <div className="text-center mb-4">
          <div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/40 bg-blue-950/40 px-4 py-2 text-cyan-100 text-xs font-bold tracking-widest">
            GAME BARU · FULL COLOR · TEMPUR LAUT
          </div>
          <h1 className="font-display text-2xl md:text-4xl font-bold text-primary text-glow-cyan mt-3">
            🚢 Kapal Selam Math Battle
          </h1>
          <p className="font-body text-sm text-white/65 mt-2 max-w-2xl mx-auto">
            Kendalikan kapal selam, tembak pesawat pembom di atas, dan hancurkan kapal selam musuh dari kiri-kanan dengan torpedo!
          </p>
        </div>

        <div className="relative rounded-[30px] p-3 bg-gradient-to-br from-cyan-300 via-blue-500 to-fuchsia-400 shadow-[0_0_50px_rgba(34,211,238,0.35)]">
          <canvas
            ref={canvasRef}
            width={CW}
            height={CH}
            ...
            className="w-[min(460px,92vw)] h-auto rounded-[22px] bg-slate-950 cursor-crosshair select-none touch-none border-4 border-slate-900"
          />
        </div>
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-3">
          ...
          <button
            onClick={() => { fireSide(-1); }}
            className="rounded-full border border-cyan-300/30 bg-cyan-400/15 px-5 py-3 text-sm font-bold text-cyan-100 hover:bg-cyan-400/25 transition-colors"
          >
            Torpedo Kiri
          </button>
          <button

...
const makeQ = (): MQ => {
  ...
  switch (t) {
    ...
    case 1: { const a = 20 + Math.floor(Math.random() * 80), b = 10 + Math.floor(Math.random() * 50); return { q: `${a} + ${b}`, ans: a + b }; }
    case 2: { const a = 30 + Math.floor(Math.random() * 70), b = 5 + Math.floor(Math.random() * 25); return { q: `${a + b} − ${b}`, ans: a }; }
    case 3: { const b = 2 + Math.floor(Math.random() * 9), a = b * (2 + Math.floor(Math.random() * 9)); return { q: `${a} ÷ ${b}`, ans: a / b }; }
    case 4: { const sq = [4,9,16,25,36,49,64,81,100,121,144][Math.floor(Math.random() * 11)]; return { q: `√${sq}`, ans: Math.round(Math.sqrt(sq)) }; }
    case 5: { const a = 2 + Math.floor(Math.random() * 9), p = 2 + Math.floor(Math.random() * 3); return { q: `${a}^${p}`, ans: Math.pow(a, p) }; }
    case 6: { const a = 50 + Math.floor(Math.random() * 51), b = 10 + Math.floor(Math.random() * 41); return { q: `${Math.max(a,b)} − ${Math.min(a,b)}`, ans: Math.abs(a - b) }; }
    default: { const a = 1 + Math.floor(Math.random() * 12), b = 1 + Math.floor(Math.random() * 12); return { q: `${a}² + ${b}²`, ans: a * a + b * b }; }
  }
};
        ...
        asteroidsRef.current.forEach(a => {
          ...
          if (Math.sqrt(dx*dx+dy*dy) < AST_R) {
            ...
            if (a.correct) {
              ...
              showFeedback("🎯 TEPAT! Asteroid hancur!", true);
              newQuestion();
        ...
        if (!a.hit) {
          ...
          if (a.y - AST_R > CH) {
            ...
            if (a.correct) {
              ...
              showFeedback("💥 Asteroid lolos! Nyawa berkurang!", false);
          ...
          {phase === "idle" && (
            <div className="absolute inset-0 flex items-center justify-center rounded-2xl bg-black/70">
              <div className="text-center px-5 max-w-xs">
                ...
                <h2 className="font-display text-2xl font-bold text-cyan-400 mb-2">ASTEROID BLASTER</h2>
                <p className="text-white/65 text-xs mb-4 leading-relaxed">
                  Tembak asteroid yang membawa angka <span className="text-yellow-400 font-bold">✨ jawaban benar</span>!<br />
                  Tembak yang salah: <span className="text-red-400">−5 poin</span><br />
                  Asteroid jawaban lolos: <span className="text-red-400">nyawa berkurang</span>
                </p>
                <div className="flex justify-center gap-2 mb-4 text-xs flex-wrap">
                  <span className="bg-yellow-500/20 border border-yellow-500/40 rounded-lg px-2 py-1">✨ Asteroid emas = jawaban benar</span>
                ...
                <p className="text-white/40 text-xs mb-5">
                  ...
                  Mobile: sentuh & geser + tombol TEMBAK
                </p>

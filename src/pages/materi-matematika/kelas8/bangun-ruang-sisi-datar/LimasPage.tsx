...
const PythagorasLimasSegitigaOverview = () => (
    <div className="bg-slate-900/70 border border-emerald-700/40 rounded-xl p-3 space-y-3">
      <div>
        ...
        <p className="text-white/45 text-[11px]">Alas segitiga sama sisi dengan sisi <InlineMath math="a" />. Rusuk atas BC dibuat horizontal. Apotema <InlineMath math="l" /> turun ke titik tengah M rusuk kanan AC.</p>
      </div>
      <svg viewBox="0 0 260 210" className="w-full max-w-sm mx-auto" aria-label="Pythagoras pada limas segitiga">
        ...
        {/*
          Base: A(100,175) front-bottom, B(62,112) back-left, C(196,112) back-right
          BC is HORIZONTAL at y=112 (top edge of the projected base)
          T(130,32) apex above centroid O(119,133)
        */}
        {/* Base A-B-C */}
        <polygon points="100,175 62,112 196,112" fill="#10b981" fillOpacity="0.22" stroke="#34d399" strokeWidth="2" />
        {/* Back face T-B-C (top horizontal edge) */}
        ...
        <circle cx="130" cy="32" r="4" fill="#facc15" />
        <circle cx="100" cy="175" r="3" fill="#e5e7eb" />
        <circle cx="62" cy="112" r="3" fill="#e5e7eb" />
        <circle cx="196" cy="112" r="3" fill="#e5e7eb" />
        <circle cx="119" cy="133" r="3.5" fill="#38bdf8" />
        <circle cx="148" cy="143" r="3.5" fill="#fb923c" />
...
/* ─────────────────────────────────────────────────────────────
   3 LIMAS → 1 PRISMA ANIMATION
───────────────────────────────────────────────────────────── */

function useAnimVal(target: number, dur = 750): number {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      else { curRef.current = target; rafRef.current = null; }
    };
...
const _pp = (pts: [number,number][]) => pts.map(p => p.join(',')).join(' ');

const SqPyramidShape = ({ cx, cy, col }: { cx:number; cy:number; col:string }) => {
  const b=20, d=10, h=30;
  const fr:[number,number]=[cx,cy], ri:[number,number]=[cx+b,cy-d];
  const bk:[number,number]=[cx,cy-2*d], le:[number,number]=[cx-b,cy-d];
  const ap:[number,number]=[cx,cy-2*d-h];
  return (
    <g>
      <polygon points={_pp([fr,le,ap])} fill={col} opacity={0.62} />
      <polygon points={_pp([fr,ri,ap])} fill={col} opacity={0.85} />
      <polygon points={_pp([le,bk,ap])} fill={col} opacity={0.45} />
      <polygon points={_pp([ri,bk,ap])} fill={col} opacity={0.58} />
      <polygon points={_pp([fr,ri,bk,le])} fill={col} opacity={0.22} />
      <polyline points={_pp([fr,ri,ap,le,fr])} fill="none" stroke={col} strokeWidth="0.7" opacity={0.5} />
      <line x1={ri[0]} y1={ri[1]} x2={bk[0]} y2={bk[1]} stroke={col} strokeWidth="0.7" opacity={0.4} />
      <line x1={le[0]} y1={le[1]} x2={bk[0]} y2={bk[1]} stroke={col} strokeWidth="0.7" opacity={0.35} />
      <line x1={bk[0]} y1={bk[1]} x2={ap[0]} y2={ap[1]} stroke={col} strokeWidth="0.7" opacity={0.35} strokeDasharray="2,1.5" />
    </g>
  );
};

const SqPrismShape = ({ cx, cy, col }: { cx:number; cy:number; col:string }) => {
  const b=20, d=10, h=30;
  const bFr:[number,number]=[cx,cy], bRi:[number,number]=[cx+b,cy-d];
  const bBk:[number,number]=[cx,cy-2*d], bLe:[number,number]=[cx-b,cy-d];
  const tFr:[number,number]=[cx,cy-h], tRi:[number,number]=[cx+b,cy-d-h];
  const tBk:[number,number]=[cx,cy-2*d-h], tLe:[number,number]=[cx-b,cy-d-h];
  return (
    <g>
      <polygon points={_pp([bFr,bRi,bBk,bLe])} fill={col} opacity={0.18} />
      <polygon points={_pp([bLe,bFr,tFr,tLe])} fill={col} opacity={0.58} />
      <polygon points={_pp([bFr,bRi,tRi,tFr])} fill={col} opacity={0.78} />
      <polygon points={_pp([tFr,tRi,tBk,tLe])} fill={col} opacity={0.95} />
      <polyline points={_pp([bFr,bRi,tRi,tFr,bFr])} fill="none" stroke={col} strokeWidth="0.9" opacity={0.6} />
      <polyline points={_pp([bLe,bFr,tFr,tLe,bLe])} fill="none" stroke={col} strokeWidth="0.9" opacity={0.45} />
      <polyline points={_pp([tFr,tRi,tBk,tLe])} fill="none" stroke={col} strokeWidth="0.9" opacity={0.55} />
      <line x1={tLe[0]} y1={tLe[1]} x2={tFr[0]} y2={tFr[1]} stroke={col} strokeWidth="0.9" opacity={0.55} />
    </g>
  );
};

const TriPyramidShape = ({ cx, cy, col }: { cx:number; cy:number; col:string }) => {
  const w=22, dpt=14, h=30;
  const bl:[number,number]=[cx-w,cy], br:[number,number]=[cx+w,cy];
  const bk:[number,number]=[cx,cy-dpt], ap:[number,number]=[cx,cy-dpt-h];
  return (
    <g>
      <polygon points={_pp([bl,br,ap])} fill={col} opacity={0.85} />
      <polygon points={_pp([br,bk,ap])} fill={col} opacity={0.65} />
      <polygon points={_pp([bl,bk,ap])} fill={col} opacity={0.48} />
      <polygon points={_pp([bl,br,bk])} fill={col} opacity={0.22} />
      <polyline points={_pp([bl,br,ap,bl])} fill="none" stroke={col} strokeWidth="0.8" opacity={0.55} />
      <line x1={br[0]} y1={br[1]} x2={bk[0]} y2={bk[1]} stroke={col} strokeWidth="0.8" opacity={0.4} strokeDasharray="2,1.5" />
      <line x1={bl[0]} y1={bl[1]} x2={bk[0]} y2={bk[1]} stroke={col} strokeWidth="0.8" opacity={0.35} strokeDasharray="2,1.5" />
      <line x1={bk[0]} y1={bk[1]} x2={ap[0]} y2={ap[1]} stroke={col} strokeWidth="0.8" opacity={0.35} strokeDasharray="2,1.5" />
    </g>
  );
};

const TriPrismShape = ({ cx, cy, col }: { cx:number; cy:number; col:string }) => {
  const w=22, dpt=14, h=30;
  const bl:[number,number]=[cx-w,cy], br:[number,number]=[cx+w,cy];
  const bk:[number,number]=[cx,cy-dpt];
  const tl:[number,number]=[cx-w,cy-h], tr:[number,number]=[cx+w,cy-h];
  const tk:[number,number]=[cx,cy-dpt-h];
  return (
    <g>
      <polygon points={_pp([bl,br,bk])} fill={col} opacity={0.18} />
      <polygon points={_pp([bl,br,tr,tl])} fill={col} opacity={0.80} />
      <polygon points={_pp([br,bk,tk,tr])} fill={col} opacity={0.62} />
      <polygon points={_pp([bl,bk,tk,tl])} fill={col} opacity={0.46} />
      <polygon points={_pp([tl,tr,tk])} fill={col} opacity={0.95} />
      <polyline points={_pp([bl,br,tr,tl,bl])} fill="none" stroke={col} strokeWidth="0.9" opacity={0.55} />
      <line x1={br[0]} y1={br[1]} x2={bk[0]} y2={bk[1]} stroke={col} strokeWidth="0.9" opacity={0.35} strokeDasharray="2,1.5" />
      <line x1={bk[0]} y1={bk[1]} x2={tk[0]} y2={tk[1]} stroke={col} strokeWidth="0.9" opacity={0.35} strokeDasharray="2,1.5" />
      <polyline points={_pp([tr,tk,tl])} fill="none" stroke={col} strokeWidth="0.9" opacity={0.55} />
    </g>
  );
};

const TigaLimasMembentukPrismaAnim = () => {
  const [type, setType] = useState<'segiempat' | 'segitiga'>('segiempat');
  const [phase, setPhase] = useState(0);
  const [playing, setPlaying] = useState(true);

  useEffect(() => {
    if (!playing) return;
    const durs = [2800, 950, 3000];
    const t = setTimeout(() => setPhase(p => (p + 1) % 3), durs[phase]);
    return () => clearTimeout(t);
  }, [phase, playing]);

  const isMerging = phase >= 1;
  const showPrism = phase === 2;

  const p1xTarget = isMerging ? 75 : 0;
  const p3xTarget = isMerging ? -75 : 0;
  const p1x = useAnimVal(p1xTarget);
  const p3x = useAnimVal(p3xTarget);

  const opPyramids = useAnimVal(showPrism ? 0 : 1, 450);
  const opPrism    = useAnimVal(showPrism ? 1 : 0, 450);
  const opPlus     = useAnimVal(isMerging ? 0 : 1, 300);

  const cols = ['#3b82f6', '#a855f7', '#22c55e'] as const;
  const prismCol = '#f59e0b';
  const cy = 100;

  const Pyramid = type === 'segiempat' ? SqPyramidShape : TriPyramidShape;
  const Prism   = type === 'segiempat' ? SqPrismShape   : TriPrismShape;

  const phaseLabel = showPrism
    ? '✓  3 Limas = 1 Prisma !'
    : isMerging
      ? '⟶  Menggabungkan...'
      : '3 Limas (alas & tinggi sama)';

  return (
    <div className="bg-slate-900/70 border border-violet-800/40 rounded-xl p-4 space-y-3">
      <p className="text-center text-xs font-bold text-violet-300 font-body tracking-wide">
        🔷 ANIMASI: 3 Limas Membentuk 1 Prisma
      </p>

      {/* Type tabs */}
      <div className="flex gap-2 justify-center">
        {(['segiempat', 'segitiga'] as const).map(tp => (
          <button
            key={tp}
            onClick={() => { playPopSound(); setType(tp); setPhase(0); setPlaying(true); }}
            className="text-xs font-bold py-1.5 px-3 rounded-lg border transition-all font-body"
            style={{
              borderColor: type === tp ? '#7c3aed' : '#334155',
              color: type === tp ? '#fff' : '#475569',
              backgroundColor: type === tp ? '#3b1f6e' : 'transparent',
            }}
          >
            {tp === 'segiempat' ? '⬛ Limas Segiempat' : '△ Limas Segitiga'}
          </button>
        ))}
      </div>
      ...
      <div className="bg-slate-800/50 rounded-lg overflow-hidden">
        <svg viewBox="0 0 280 130" className="w-full" style={{ maxHeight: 200 }}>
          {/* Phase status */}
          <text x={140} y={13} textAnchor="middle" fontSize="8.5"
            fill={showPrism ? '#fbbf24' : '#64748b'} fontFamily="monospace" fontWeight="bold">
            {phaseLabel}
          </text>

          {/* ── 3 Pyramids (phases 0 & 1) ── */}
          <g opacity={opPyramids}>
            {/* Pyramid 1 */}
            <g transform={`translate(${p1x.toFixed(2)}, 0)`}>
              <Pyramid cx={40} cy={cy} col={cols[0]} />
              <text x={40} y={cy+14} textAnchor="middle" fontSize="7" fill={cols[0]} fontFamily="monospace">Limas 1</text>
            </g>

            {/* Plus 1 */}
            <text x={78} y={76} textAnchor="middle" fontSize="17" fill="#334155"
              fontFamily="sans-serif" fontWeight="bold" opacity={opPlus}>+</text>

            {/* Pyramid 2 (stays at center) */}
            <Pyramid cx={115} cy={cy} col={cols[1]} />
            <text x={115} y={cy+14} textAnchor="middle" fontSize="7" fill={cols[1]}
              fontFamily="monospace" opacity={opPlus}>Limas 2</text>

            {/* Plus 2 */}
            <text x={153} y={76} textAnchor="middle" fontSize="17" fill="#334155"
              fontFamily="sans-serif" fontWeight="bold" opacity={opPlus}>+</text>

            {/* Pyramid 3 */}
            <g transform={`translate(${p3x.toFixed(2)}, 0)`}>
              <Pyramid cx={190} cy={cy} col={cols[2]} />
              <text x={190} y={cy+14} textAnchor="middle" fontSize="7" fill={cols[2]} fontFamily="monospace">Limas 3</text>
            </g>
          </g>
          ...
          <g opacity={opPrism}>
            {/* glow ring */}
            <circle cx={115} cy={cy-22} r={33} fill="none" stroke={prismCol} strokeWidth="1" opacity={0.2} />
            <Prism cx={115} cy={cy} col={prismCol} />
            <text x={115} y={cy+14} textAnchor="middle" fontSize="8" fill={prismCol}
              fontFamily="monospace" fontWeight="bold">1 PRISMA</text>
          </g>

          {/* bottom equation */}
          <text x={140} y={126} textAnchor="middle" fontSize="8" fill="#94a3b8" fontFamily="monospace">
            3 × (⅓ × L × t) = L × t = V_prisma
          </text>
        </svg>
      </div>
      ...
      <div className="flex justify-center gap-5">
        <button onClick={() => setPlaying(p => !p)}
          className="text-xs text-slate-400 hover:text-white transition-colors font-body">
          {playing ? '⏸ Pause' : '▶ Play'}
        </button>
        <button onClick={() => { setPhase(0); setPlaying(true); }}
          className="text-xs text-slate-400 hover:text-white transition-colors font-body">
          ↺ Ulang
        </button>
      </div>
      ...
      {/* Insight box */}
      <div className="bg-violet-950/40 border border-violet-700/30 rounded-lg p-3 text-xs space-y-2 font-body">
        <p className="text-violet-300 font-semibold">💡 Kesimpulan:</p>
        <p className="text-white/80">
          3 buah limas dengan <strong className="text-yellow-300">alas dan tinggi yang sama</strong> tepat mengisi 1 prisma dengan alas dan tinggi yang sama.
        </p>
        <div className="grid grid-cols-2 gap-2 text-xs mt-1">
          <div className="bg-slate-800/60 border border-blue-700/30 rounded p-2 text-center">
            <p className="text-blue-300 font-semibold mb-1">△ Limas Segitiga</p>
            <p className="text-white/70">3 limas segitiga</p>
            <p className="text-yellow-300 font-bold">= 1 prisma segitiga</p>
          </div>
          <div className="bg-slate-800/60 border border-amber-700/30 rounded p-2 text-center">
            <p className="text-amber-300 font-semibold mb-1">⬛ Limas Segiempat</p>
            <p className="text-white/70">3 limas segiempat</p>
            <p className="text-yellow-300 font-bold">= 1 prisma segiempat</p>
          </div>
        </div>
        <div className="bg-slate-900/60 rounded p-2">
          <BlockMath math="V_{\text{limas}} = \frac{1}{3} \times V_{\text{prisma}} \Longrightarrow 3 \times V_{\text{limas}} = V_{\text{prisma}}" />
        </div>
      </div>
    </div>
  );

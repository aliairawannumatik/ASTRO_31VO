...
/* ─────────────────────────────────────────────────────────────
   SELIMUT UNROLL ANIMATION
   RAF-driven: progress 0→1 controls both the unrolling rectangle
   and a roll-edge indicator that shrinks as the surface flattens.
───────────────────────────────────────────────────────────── */
const _easeOutCubic = (t: number) => 1 - Math.pow(1 - t, 3);
const UNROLL_DURATION = 1600; // ms

const SelimutUnrollAnimation = () => {
  ...
  const doStart = () => {
    ...
    const tick = (now: number) => {
      ...
      const raw = Math.min((now - startRef.current) / UNROLL_DURATION, 1);
      const p   = _easeOutCubic(raw);
      setProgress(p);
      if (raw < 1) {
  ...
  /* Roll edge: 28px at p=0 → 0px at p=1 */
  const rollW   = Math.max(0, 28 * (1 - progress));
  const rollOp  = Math.max(0, 1 - progress * 1.15);
  /* Rectangle fills from left; roll sits right at the leading edge */
  const rectPct = `${(progress * 100).toFixed(1)}%`;
  ...
  return (
    <div style={{
      ...
      <style>{`
        @keyframes su-arrow-pulse {
          0%,100% { opacity:.3; transform:translateX(0); }
          50%      { opacity:1;  transform:translateX(3px); }
        }
        @keyframes su-formula-in {
          from { opacity:0; transform:translateY(6px); }
          to   { opacity:1; transform:translateY(0); }
        }
        .su-arrow-active { animation: su-arrow-pulse .75s ease-in-out infinite; }
        .su-formula-in   { animation: su-formula-in .45s ease-out both; }
      `}</style>

      {/* ── header ── */}
      <p style={{ textAlign:"center", fontFamily:"monospace", fontSize:10, fontWeight:"bold",
        color:"#a78bfa", letterSpacing:".07em", textTransform:"uppercase", marginBottom:12 }}>
        ✨ Animasi: Selimut Tabung → Persegi Panjang
      </p>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
        ...
        <div style={{ flex:"0 0 100px" }}>
          <svg viewBox="0 0 100 182" width="100" style={{ display:"block", overflow:"visible" }}>
            {/* back arc dashed */}
            <path d="M 10,42 A 40,11 0 0,0 90,42"
              fill="none" stroke="rgba(168,85,247,0.25)" strokeWidth="1" strokeDasharray="4,2"/>
            {/* selimut body — fades as progress grows */}
            <path d="M 10,42 A 40,11 0 0,1 90,42 L 90,146 A 40,11 0 0,0 10,146 Z"
              fill="rgba(168,85,247,0.40)"
              stroke="#a855f7" strokeWidth="1.8"
              style={{ opacity: 1 - progress * 0.85,
                       transition: isActive ? "none" : "opacity .3s" }}
            />
            {/* bottom cap */}
            <ellipse cx="50" cy="146" rx="40" ry="11"
              fill="rgba(99,102,241,0.45)" stroke="#818cf8" strokeWidth="1.5"/>
            {/* top cap */}
            <ellipse cx="50" cy="42" rx="40" ry="11"
              fill="rgba(99,102,241,0.55)" stroke="#818cf8" strokeWidth="1.5"/>
            {/* r */}
            <line x1="50" y1="42" x2="90" y2="42" stroke="#f59e0b" strokeWidth="1.4" strokeDasharray="3,2"/>
            <circle cx="50" cy="42" r="2.2" fill="#f59e0b"/>
            <circle cx="90" cy="42" r="2.2" fill="#f59e0b"/>
            <text x="70" y="36" fill="#f59e0b" fontSize="9.5" fontFamily="monospace" fontWeight="bold" textAnchor="middle">r</text>
            {/* t */}
            <line x1="94" y1="42" x2="94" y2="146" stroke="#22c55e" strokeWidth="1.4"/>
            <line x1="90" y1="42"  x2="98" y2="42"  stroke="#22c55e" strokeWidth="1.2"/>
            <line x1="90" y1="146" x2="98" y2="146" stroke="#22c55e" strokeWidth="1.2"/>
            <text x="100" y="98" fill="#22c55e" fontSize="10" fontFamily="monospace" fontWeight="bold" textAnchor="middle">t</text>
            {/* selimut label fades out */}
            <text x="50" y="98" fill="#e9d5ff" fontSize="8.5" fontFamily="monospace" fontWeight="bold"
              textAnchor="middle"
              style={{ opacity: Math.max(0, 1 - progress * 1.4) }}>SELIMUT</text>
            <text x="50" y="170" fill="#6d28d9" fontSize="8" fontFamily="monospace" textAnchor="middle">Tabung</text>
          </svg>
        </div>

        {/* ARROW */}
        <div style={{ flex:"0 0 22px", textAlign:"center" }}>
          <div className={isActive && !isDone ? "su-arrow-active" : ""}
            style={{ fontSize:18, color:"#a855f7", opacity: isActive ? 1 : .28 }}>
            →
          </div>
          <div style={{ fontSize:6.5, color:"#7c3aed", fontFamily:"monospace", marginTop:2 }}>
            {isDone ? "terbuka!" : isActive ? "membuka…" : "buka"}
          </div>
        </div>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {/* top dimension label */}
          <div style={{ textAlign:"center", fontSize:8.5, fontFamily:"monospace", color:"#a855f7",
            marginBottom:4,
            opacity: isDone ? 1 : 0, transition: isDone ? "opacity .4s .1s" : "none" }}>
            ←&nbsp;&nbsp;lebar&nbsp;=&nbsp;2πr&nbsp;&nbsp;→
          </div>
          ...
          <div style={{ display:"flex", alignItems:"stretch", gap:5 }}>
            <div style={{ flex:1, position:"relative", height:108 }}>
              ...
              {/* ghost placeholder (dashed full-width outline) */}
              <div style={{
                position:"absolute", inset:0,
                border:"1.5px dashed rgba(168,85,247,0.22)",
                borderRadius:6,
                opacity: isDone ? 0 : 0.7,
                transition: isDone ? "opacity .5s" : "none",
              }}/>

              {/* unrolled rectangle — grows from left */}
              <div style={{
                position:"absolute",
                top:0, left:0, bottom:0,
                width: rectPct,
                minWidth: isActive ? 2 : 0,
                borderRadius: isDone ? 6 : "6px 0 0 6px",
                overflow:"hidden",
                /* gradient: right edge darker/deeper to simulate curving surface */
                background: isDone
                  ? "linear-gradient(135deg,rgba(168,85,247,.30) 0%,rgba(124,58,237,.18) 100%)"
                  : `linear-gradient(to right,
                      rgba(168,85,247,.32) 0%,
                      rgba(168,85,247,.32) ${Math.max(0,(progress-0.12)*100/0.88).toFixed(0)}%,
                      rgba(90,30,180,.70) 95%,
                      rgba(60,10,140,.90) 100%)`,
                border: "2px solid #a855f7",
                borderRight: isDone ? "2px solid #a855f7" : "none",
                transition: isDone ? "background .4s, border .1s" : "none",
              }}>
                {/* inner label — appears when done */}
                <div style={{
                  position:"absolute", inset:0,
                  display:"flex", alignItems:"center", justifyContent:"center",
                  flexDirection:"column", gap:3,
                  opacity: isDone ? 1 : 0,
                  transition: isDone ? "opacity .5s .2s" : "none",
                }}>
                  <span style={{ fontFamily:"monospace", fontSize:10, fontWeight:"bold", color:"#e9d5ff" }}>
                    SELIMUT TABUNG
                  </span>
                  <span style={{ fontFamily:"monospace", fontSize:8.5, color:"#c4b5fd" }}>
                    Persegi Panjang
                  </span>
                </div>
              </div>

              {/* ROLL EDGE INDICATOR — sits right at the leading edge, shrinks to 0 */}
              {isActive && rollW > 0.5 && (
                <div style={{
                  position:"absolute",
                  top:0, bottom:0,
                  left: `calc(${rectPct} - ${rollW * 0.35}px)`,
                  width: rollW,
                  borderRadius: "0 50% 50% 0 / 0 40% 40% 0",
                  background: `linear-gradient(to right,
                    rgba(168,85,247,0.55) 0%,
                    rgba(140,60,230,0.85) 40%,
                    rgba(100,20,180,1.00) 65%,
                    rgba(60,5,120,0.90)  100%)`,
                  boxShadow: "2px 0 12px rgba(168,85,247,0.6)",
                  opacity: rollOp,
                  zIndex: 2,
                }}/>
              )}

              {/* thin bright leading line while animating */}
              {isActive && !isDone && (
                <div style={{
                  position:"absolute",
                  top:0, bottom:0,
                  left: `calc(${rectPct} - 1.5px)`,
                  width: 3,
            ...
            {/* t dimension indicator (right side) */}
            <div style={{
              display:"flex", flexDirection:"column", alignItems:"center", gap:2,
              opacity: isDone ? 1 : .15, transition:"opacity .5s .1s",
            }}>
              <div style={{ flex:1, width:1.5, background:"#22c55e", minHeight:10 }}/>
              <span style={{ fontSize:11, color:"#22c55e", fontFamily:"monospace", fontWeight:"bold", lineHeight:1 }}>t</span>
              <div style={{ flex:1, width:1.5, background:"#22c55e", minHeight:10 }}/>
            </div>
          </div>
          ...
          <div style={{ marginTop:7, textAlign:"center",
            opacity: isDone ? 1 : 0, transition: isDone ? "opacity .4s .3s" : "none" }}>
            {isDone && (
              <span className="su-formula-in" style={{
                display:"inline-block",
                background:"rgba(168,85,247,.18)", border:"1px solid rgba(168,85,247,.55)",
                borderRadius:8, padding:"5px 14px",
                fontSize:11, fontFamily:"monospace", fontWeight:"bold", color:"#ddd6fe",
              }}>
                Luas Selimut&nbsp;=&nbsp;2πr&nbsp;×&nbsp;t
              </span>
            )}
      ...
      <div style={{ display:"flex", gap:8, justifyContent:"center", marginTop:13 }}>
        <button onClick={doStart} disabled={phase !== "idle"}
          style={{
            ...
            background: phase === "idle" ? "rgba(168,85,247,.18)" : "transparent",
            color:"#d8b4fe", fontSize:12, fontWeight:"bold",

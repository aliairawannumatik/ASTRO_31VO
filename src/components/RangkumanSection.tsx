    ...
    <div className="px-5 pb-7 pt-5 space-y-6">
          ...
          {ringkasan.map((r, i) => (
            <div key={i} className={`${r.bg} border ${r.border} rounded-xl p-3 flex gap-3 items-start`}>
              ...
              <div>
                ...
                <p className="text-white/70 text-xs font-body leading-relaxed">{r.isi}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {tips.map((t, i) => (
            <div key={i} className="bg-amber-500/10 border border-amber-500/30 rounded-xl px-4 py-2.5 flex gap-3 items-start">
              <span className="text-base shrink-0 mt-0.5">{t.emoji}</span>
              <p className="text-amber-100 text-xs font-body leading-relaxed">{t.teks}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className={`${kesimpulanBg} border ${kesimpulanBorder} rounded-2xl px-5 py-4 text-center`}>
        ...
        <p className="text-white font-semibold text-sm font-body mb-1">Kesimpulan</p>

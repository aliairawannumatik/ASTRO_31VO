      ...
      <div className="grid md:grid-cols-[1fr_220px] gap-4 items-start">
        ...
        <div className="relative">
          ...
          >
            ...
            {[-6, -4, -2, 2, 4, 6].map((v) => (
              <g key={`l${v}`}>
                <text x={toSVGX(v)} y={O + 12} fill="#64748b" fontSize="9" textAnchor="middle">
                  {v}
                </text>
                <text x={O - 6} y={toSVGY(v) + 3} fill="#64748b" fontSize="9" textAnchor="end">
                  {v}
                </text>
              </g>
            ))}
            ...
            {eq1Endpoints.length === 2 && (
              <line
                x1={toSVGX(eq1Endpoints[0].x)}
                y1={toSVGY(eq1Endpoints[0].y)}
                x2={toSVGX(eq1Endpoints[1].x)}
                y2={toSVGY(eq1Endpoints[1].y)}
                stroke={system.eq1.color}
                strokeWidth={3}
                strokeLinecap="round"
                style={{ filter: system.eq1.glow }}
              />
            )}
            {/* Line 2 */}
            {eq2Endpoints.length === 2 && (
              <line
                x1={toSVGX(eq2Endpoints[0].x)}
                y1={toSVGY(eq2Endpoints[0].y)}
                x2={toSVGX(eq2Endpoints[1].x)}
                y2={toSVGY(eq2Endpoints[1].y)}
                stroke={system.eq2.color}
                strokeWidth={3}
                strokeLinecap="round"
                style={{ filter: system.eq2.glow }}
              />
            )}

            {/* Intersection halo when solved */}
            {isSolution && (
              <>
                <circle
                  cx={toSVGX(system.solution.x)}
                  cy={toSVGY(system.solution.y)}
                  r={20}
                  fill="none"
                  stroke="#fde047"
                  strokeWidth={2}
            ...
            <circle
              ...
              cy={toSVGY(point.y)}
              r={11}
              fill={isSolution ? "#facc15" : "#f8fafc"}
              stroke={isSolution ? "#fde047" : "#22d3ee"}
              strokeWidth={3}
              onPointerDown={handleDown}
              onPointerUp={handleUp}
              style={{ cursor: dragging ? "grabbing" : "grab", filter: "drop-shadow(0 0 8px rgba(34,211,238,0.6))" }}
            />
            <text
              x={toSVGX(point.x) + 14}
              y={toSVGY(point.y) - 12}
              fill={isSolution ? "#fde047" : "#e2e8f0"}
              fontSize="12"
              fontWeight="bold"
          ...
          <p className="mt-2 text-[11px] text-center text-white/55 font-body italic">
            🖱️ Seret titik putih ke perpotongan dua garis untuk menemukan solusinya.
          </p>
        </div>
        ...
        <div className="space-y-2">
          <div className="rounded-xl border border-white/15 bg-black/30 p-3">
            <p className="text-[10px] uppercase tracking-wider text-white/50 mb-1">Sistem Persamaan</p>
          ...
          <div className="rounded-xl border border-white/15 bg-black/30 p-3 space-y-1.5">
            <p className="text-[10px] uppercase tracking-wider text-white/50">Cek Substitusi</p>
            <div
              className={`rounded-lg px-2 py-1.5 text-xs font-bold ${
                onLine1 ? "bg-emerald-500/20 text-emerald-200 border border-emerald-300/40" : "bg-white/5 text-white/70 border border-white/10"
              }`}
            >
              <span className="opacity-70">Pers 1: </span>
              {system.eq1.a}({point.x}) {system.eq1.b >= 0 ? "+" : "−"} {Math.abs(system.eq1.b)}({point.y}) = {lhs1}
              {onLine1 ? " ✓" : ` ≠ ${system.eq1.c}`}
            </div>
            <div
              className={`rounded-lg px-2 py-1.5 text-xs font-bold ${
                onLine2 ? "bg-emerald-500/20 text-emerald-200 border border-emerald-300/40" : "bg-white/5 text-white/70 border border-white/10"
              }`}
            >
              <span className="opacity-70">Pers 2: </span>
          ...
          {isSolution ? (
            <div className="rounded-xl border-2 border-yellow-300/60 bg-gradient-to-br from-yellow-400/20 to-amber-500/20 p-3 text-center animate-pulse">
              ...
              <p className="text-sm font-bold text-yellow-100">SOLUSI DITEMUKAN!</p>
              <p className="text-xs text-yellow-200 mt-1">
                ({system.solution.x}, {system.solution.y}) memenuhi keduanya.
              </p>
            </div>
          ) : (
            <div className="rounded-xl border border-cyan-300/30 bg-cyan-500/10 p-3 text-center">
              <p className="text-xs text-cyan-100 font-body">
                {onLine1 && !onLine2 && "Sudah pas di garis 1, geser sedikit agar pas di garis 2 juga."}
                {onLine2 && !onLine1 && "Sudah pas di garis 2, geser sedikit agar pas di garis 1 juga."}
                {!onLine1 && !onLine2 && "Geser titik ke arah perpotongan dua garis."}
              </p>
            </div>
          )}
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={snapToSolution}
              className="flex-1 rounded-lg bg-emerald-500/80 hover:bg-emerald-500 text-white text-xs font-bold py-2 transition-colors"
            >
              💡 Tunjukkan
            </button>
            <button
              type="button"
              onClick={resetPoint}
              className="flex-1 rounded-lg border border-white/20 bg-white/5 hover:bg-white/10 text-white text-xs font-bold py-2 transition-colors"
            >
              🔄 Ulang
            </button>
          </div>

...
- Caught mid-task: `SphereFruitCutAnimation` and `WaterBolaAnimation` (used live in Slides 3–4) had hardcoded Indonesian button/status text not covered by the earlier "Slide 3/4 done" note — added a `language` prop to both and gated their text, confirming the general lesson below about animation sub-components not inheriting page-level language state automatically.
</content>

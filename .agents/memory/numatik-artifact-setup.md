...
- Confirmed a third time on 2026-07-15: even though `artifacts/{numatik,api-server,mockup-sandbox}/.replit-artifact/artifact.toml` files exist on disk, `listArtifacts()`/`WorkflowsRestart` with artifact-managed names (`artifacts/numatik: web`, `artifacts/api-server: API Server`) still fail with "doesn't exist in config" every time on a fresh import — registration never survives. Don't waste a round-trip probing those names; go straight to `configureWorkflow` for the two manual workflows.

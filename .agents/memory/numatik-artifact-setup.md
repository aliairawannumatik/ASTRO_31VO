...
## KerucutPage (materi-matematika) trilingual — COMPLETE (all 11 slides)
All slides (1-11, after inserting a new "Contoh Soal — Garis Pelukis" slide before the Luas/Volume example slides) are fully trilingual (id/en/ja), TypeScript-clean, verified via `tsc --noEmit` + spot-checked externalUrl screenshot. Pattern used throughout: split each example set into a language-independent `*Meta` array (styling: level/color/bg/border/badgeBg) + a `Record<Language, ExContent[]>` translation object (question/answer JSX), merged via a `get*Examples(language)` helper — avoids the "key from wrong object" blank-screen bug. Example-card badge prefixes (LUAS/VOLUME/GARIS PELUKIS) and the show/hide-solution toggle text are now both driven by shared translation objects (`exampleSlidesTrans[language].*Prefix`, `toggleLabelsTrans[language]`) instead of hardcoded Indonesian strings, and `UnsurSoalQuiz`'s toggle was unified onto the same `toggleLabelsTrans` (dropped its own separate, inconsistent "Sembunyikan Pembahasan" wording).
**Why:** this file was translated incrementally across many sessions; this closes out the last known gaps (hardcoded prefixes, inconsistent toggle text, untranslated Garis Pelukis examples, remaining `\text{...}` KaTeX subscripts).
**How to apply:** treat this file as a reference pattern for other bangun-ruang-sisi-lengkung pages (TabungPage already follows the same convention) — reuse the Meta/Trans split and shared toggle/prefix translation objects rather than re-inventing per-page.

...
- **Content Features**: Content pages extensively use `react-katex` for LaTeX rendering, SVG diagrams for visual explanations, and interactive components like accordions and collapsible sections. The Olimpiade pages (Persamaan Kuadrat, Fungsi Kuadrat, Aritmetika Sosial) include per-question "Lihat Pembahasan" toggles that reveal the answer key (emerald box) and step-by-step LaTeX solution (cyan/purple box) on both Latihan Dasar and Latihan Olimpiade tabs. The newer Olimpiade pages (SPLDV, Pola Bilangan) use a richer 5-section colored discussion card (`PembahasanCard`): Jawaban (emerald), Konsep & Trik (violet), Step by Step (cyan), Tips (amber), Kesimpulan (rose). Pembahasan data is stored in `src/data/pembahasan/*.ts` keyed by question number.

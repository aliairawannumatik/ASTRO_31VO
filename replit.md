...
- **Content Features**: Content pages extensively use `react-katex` for LaTeX rendering, SVG diagrams for visual explanations, and interactive components like accordions and collapsible sections. The Olimpiade pages (Persamaan Kuadrat, Fungsi Kuadrat, Aritmetika Sosial) include per-question "Lihat Pembahasan" toggles that reveal the answer key (emerald box) and step-by-step LaTeX solution (cyan/purple box) on both Latihan Dasar and Latihan Olimpiade tabs.

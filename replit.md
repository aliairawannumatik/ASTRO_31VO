...
- Gamification: 7-game arena variants per submenu (Meteor, Flappy Rocket, Shoot Tank, Space Impact, Turtle Run, Tetris, Snake)
  - **Pause-quiz pop-up (`useGuruQuiz`):** Each game (`umum/{FlappyRocketPage,BattleTankPage,SpaceImpactPage,DinoRunGamePage,TetrisGamePage,SnakeMathPage}`) accepts a `quizQuestions?: GuruQuestion[]` prop forwarded to `useGuruQuiz`. K7/K8/K9 dispatchers pass `q.snake` as `quizQuestions` so the pop-up matches the active sub-materi. Bilangan-bulat static wrappers import per-subtopic question banks from `src/data/mga-k7-bilbul-quiz.ts` (`PENJUMLAHAN_QUIZ`, `PENGURANGAN_QUIZ`, `PERKALIAN_QUIZ`, `PEMBAGIAN_QUIZ`, `OPERASI_CAMPURAN_QUIZ`, `KPK_FPB_QUIZ`). Meteor game does not use `useGuruQuiz`.
